...
# Removed '.prose' from Claude so it stops capturing your own user prompt.
BOT_CONFIGS = {
    "ChatGPT": {"url": "https://chatgpt.com/", "input": "#prompt-textarea", "assistant": "div[data-message-author-role='assistant']"},
    "Claude": {"url": "https://claude.ai/", "input": "div[contenteditable='true']", "assistant": ".font-claude-message, div[data-message-author-role='assistant'], div[data-test-render-count]"},
...
class ChatbotAdapter:
    ...
    async def submit_and_get_response(self, prompt: str, *, submit_only: bool=False) -> Optional[str]:
        ...
        try:
            ...
            # The Ultimate React Input Hack: Fill all but the last character, then manually physically 'type' the last one 
            # to trigger the browser's native KeyboardEvents so the Send button unlocks.
        ...
        send_selectors = [
            'button[aria-label*="Submit" i]', 'button[data-testid*="send" i]',
            'button[aria-label*="Send" i]', 'div[role="button"][aria-label*="Send" i]',
